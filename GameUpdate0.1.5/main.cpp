...
int main() {
	...
	sf::RenderWindow window(sf::VideoMode::getDesktopMode(), "BLUEMAN IN WONDERLAND", sf::Style::Close | sf::Style::Titlebar);
	...
	while (window.isOpen()) {
		...
		if (state == MAINMENU_S) {
			...
			if (gameplay->isVictory == 0) {
				...
				t.create(1920, 1080);
				t.update(window);
				t.copyToImage().saveToFile("gameover.png");
