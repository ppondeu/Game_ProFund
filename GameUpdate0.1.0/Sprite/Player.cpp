...
Player::Player() {
	...
	bulletSoundBuffer.loadFromFile("Audio/bulletSound.wav");
...
void Player::checkIsPlaySound() {
	if (!bulletSoundS.empty()) {
		for (int i = 0; i < bulletSoundS.size(); i++) {
			if (bulletSoundS.at(i).getStatus() == 0) indexBulletSoundErase.push(i);
		}
	}
	while (!indexBulletSoundErase.empty()) {
		bulletSoundS.erase(bulletSoundS.begin() + indexBulletSoundErase.front());
		indexBulletSoundErase.pop();
	}
}
