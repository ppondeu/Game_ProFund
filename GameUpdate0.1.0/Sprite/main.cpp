...
int main(){
	...
	std::vector<short> *amountEnemy = new std::vector<short>{ 30, 20 , 15};
	float delay = 3.0f;
	int cntPlayer = 0;
	while (window.isOpen()) {
		...
		if (isSpawn) {
			...
			if (cntDelay >= delay) {
				...
				//std::cout << amountEnemy[0] <<" " << amountEnemy[1] << " " << amountEnemy[2] << " "  << std::endl;
