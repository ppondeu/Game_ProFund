...
PlayerII::PlayerII() {
	...
	this->playerSP.setPosition(500, 500);
	this->bodySP.setPosition(500, 500);
	...
	// temp crop enemy Sprite
	this->rect.setSize(sf::Vector2f(imageSize.x * scale.x, imageSize.y *scale.y));
...
void PlayerII::_move() {
	this->playerSP.move(this->move);
	this->bodySP.move(this->move);
}

void PlayerII::resetFrameExceptIndex(const int top) {
	for (int i = 0; i < this->maxTop; i++) if (i != top) this->left[i] = 0;
}

void PlayerII::setModeSideTop(const int mode, const int side, const int top, const sf::Vector2f move) {
	this->mode = mode;
	this->side = side;
	this->top = top;
	this->move = move;
...
void PlayerII::_setFrame(const int mode) {
	if (mode == this->mode) {
		++this->left[top];
...
void PlayerII::pushBullet(const int top) {
...
void PlayerII::_popBullet(const int index) {
...
void PlayerII::_setTextureRectBullet() {
...
void PlayerII::_moveBullet() {
	for (int i = 0; i < bulletS.size(); i++) {
		if(this->sideBullet[i] == RIGHT) this->bulletS[i].move(sf::Vector2f{ speed, 0.0f });
		else if(this->sideBullet[i] == LEFT) this->bulletS[i].move(sf::Vector2f{ -speed, 0.0f });
	}
}

void PlayerII::_setBulletFrame() {
	for (int i = 0; i < bulletS.size(); i++) {
...
bool PlayerII::isBulletOnScreen(sf::Vector2f bulletPos) {
	if (bulletPos.x > 1930) return true;
	if (bulletPos.x < -56) return true;
	return false;
}

void PlayerII::clearBullet() {
...
void PlayerII::_isKeyPressed() {
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::J)) {
		++cntPressedJ;
		if (cntPressedJ == 1) {
			if (curCombo == ATK_R && energyPlayer /5 > 0) {
				if (side == RIGHT) {
			...
			else if (curCombo == EXTRA_ATK_R && energyPlayer / 7 > 0) {
				if (side == RIGHT) {
					setModeSideTop(ATK, RIGHT, curCombo, { 0.0f, 0.0f });
				}
				if (side == LEFT) {
...
void PlayerII::onEvent(sf::Event& event) {
...
void PlayerII::onUpdate(float deltaTime) {
	clearBullet();
...
void PlayerII::_draw(sf::RenderWindow& window) {
	sf::RectangleShape r;
	r.setSize(sf::Vector2f(5, 5));
	r.setPosition(playerSP.getPosition().x + imageSize.x / 2 * scale.x, playerSP.getPosition().y);
	r.setFillColor(sf::Color::Red);

	rect.setPosition(playerSP.getPosition());
	window.draw(rect);
...
unsigned short PlayerII::updateHealthPlayer() {
	unsigned short tempHealthRegen = healthRegen;
	healthRegen = 0;
	return tempHealthRegen;
}
short PlayerII::setHealthPlayer() {
	return this->healthPlayer;
}

short PlayerII::updateEnergyPlayer() {
	short tempEnergyUsed = energyUsed;
	energyUsed = 0;
	return tempEnergyUsed;
}
void PlayerII::requestDeltaHealthPlayer(const short deltaHealthPlayer) {
	...
	else if (this->healthPlayer >= 1000) this->healthPlayer = 1000;
}

void PlayerII::requestEnergyPlayer(const short energyPlayer) {
	this->energyPlayer = energyPlayer;
}

void PlayerII::_collision(const std::string condition, const bool isCollided, const int index) {
...
void PlayerII::itemAffect(const unsigned short itemSelected) {
