...
void EnemyIII::onEvent(const sf::Event& event) {
...
void EnemyIII::_draw(sf::RenderWindow& window) {
	for (int i = 0; i < enemyS.size(); i++) {
		//std::cout << mode[0] << " " << move[0].x << "," << move[0].y << std::endl;
...
void EnemyIII::setHealth(const std::string condition, const int index, const int curCombo) {
	if (condition == "PLAYER") {
		this->healthPlayer -= 11;
		this->deltaHealthPlayer += -11;
		if (healthPlayer <= 0) {
			healthPlayer = 0;
...
bool EnemyIII::updateScore() {
	return isEnemyDied;
}

unsigned short EnemyIII::updateHealthMiss() {
	if (isHealthMiss) {
		isHealthMiss = false;
		return 1;
	}
...
unsigned short EnemyIII::updateEnergyPlayer() {
	unsigned short tempEnergyRegen = energyRegen;
	energyRegen = 0;
