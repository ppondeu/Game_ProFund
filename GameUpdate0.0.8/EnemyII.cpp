...
void EnemyII::pushEnemy() {
	...
	this->enemyS.back().setPosition(1700, spawnPos[tempRandPos]);
	this->bodyS.back().setPosition(1700, spawnPos[tempRandPos]);
...
void EnemyII::clearEnemy() {
	...
	for (int i = 0; i < enemyS.size(); ++i) {
		if (!isEnemyOnScreen(i)) indexEnemyErase.insert(-i);
	}
	for (auto it = indexEnemyErase.begin(); it != indexEnemyErase.end(); ++it) {
		popEnemy(*it * -1);
	}
...
bool EnemyII::updateScore() {
	bool tempIsEnemyDied = isEnemyDied;
	isEnemyDied = false;
	return tempIsEnemyDied;
}
