...
int main() {
	...
	int idxAlpha=0;
	std::vector<std::string> alphaMovement{ "am1.png","am2.png", "am3.png",
	"am4.png", "am5.png", "am6.png", };
	sf::Texture alphaTexture;
	alphaTexture.loadFromFile(alphaMovement[idxAlpha]);
	sf::Sprite alphaSprite;
	alphaSprite.setTexture(alphaTexture);
	sf::Vector2f alphaPos{1700, 490};
	alphaSprite.setPosition(alphaPos);
	alphaSprite.setScale(1.8, 1.8);
	...
	while (window.isOpen()) {
		...
		if (static_cast<unsigned>(time3.asMilliseconds()) > 200) {
			++idxAlpha;
			if (idxAlpha >= alphaMovement.size()) {
				idxAlpha = 0;
			}
			alphaTexture.loadFromFile(alphaMovement[idxAlpha]);
			alphaSprite.setTexture(alphaTexture);
			alphaSprite.move(-18, 0);
